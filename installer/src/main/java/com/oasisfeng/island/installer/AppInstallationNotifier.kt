...
internal object AppInstallationNotifier {
	...
	fun onInstallFail(context: Context, sessionId: Int, install: AppInstallInfo, message: String) {
		...
		if (SDK_INT >= N && context.getSystemService(NotificationManager::class.java)?.areNotificationsEnabled() == false)
			Toast.makeText(context, context.getString(R.string.dialog_install_failure_title, install.appLabel) + "\n" + message,
